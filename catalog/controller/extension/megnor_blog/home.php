<?php 
class ControllerExtensionMegnorBlogHome extends Controller {
	private $error = array();
	...
	public function index() { 
		...
		$blog_total = $this->model_extension_megnor_blog_blog->getTotalBlogs($filter_data);
		
		$results = $this->model_extension_megnor_blog_blog->getBlogs($filter_data, $pagefix, $limit);
		...
		$pagination->url = $this->url->link('extension/megnor_blog/home', $url . '&page={page}');
		...
		$this->response->setOutput($this->load->view('extension/megnor_blog/blog_home', $data));
	}
}