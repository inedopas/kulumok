...
class ModelToolExportImport extends Model {
	...
	protected function storeProductOptionValueIntoDatabase( &$product_option_value, &$old_product_option_value_ids ) {
		...
		$size = $product_option_value['size'];
		$color = $product_option_value['color'];
		$form = $product_option_value['form'];
		...
		if (isset($old_product_option_value_ids[$product_id][$option_id][$option_value_id])) {
			...
			$sql .= "(`product_option_value_id`,`product_option_id`,`product_id`,`option_id`,`option_value_id`,`quantity`,`subtract`,`price`,`price_prefix`,`size`,`color`,`form`,`points`,`points_prefix`,`weight`,`weight_prefix`) VALUES "; 



			$sql .= "($product_option_value_id,$product_option_id,$product_id,$option_id,$option_value_id,$quantity,$subtract,$price,'$price_prefix','$size','$color','$form',$points,'$points_prefix',$weight,'$weight_prefix')";
			...
			$sql .= "(`product_option_id`,`product_id`,`option_id`,`option_value_id`,`quantity`,`subtract`,`price`,`price_prefix`,`size`,`color`,`form`,`points`,`points_prefix`,`weight`,`weight_prefix`) VALUES "; 


			$sql .= "($product_option_id,$product_id,$option_id,$option_value_id,$quantity,$subtract,$price,'$price_prefix','$size','$color','$form',$points,'$points_prefix',$weight,'$weight_prefix')";
			$this->db->query($sql);
		}
	}
	...
	protected function uploadProductOptionValues( &$reader, $incremental, &$available_product_ids ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$product_option_value['size'] = $size;
			$product_option_value['color'] = $color;
			$product_option_value['form'] = $form;
	...
	protected function validateProductOptionValues( &$reader ) {
		...
		if ($this->config->get('export_import_settings_use_option_id')) {
			if ($this->config->get('export_import_settings_use_option_value_id')) {
				$expected_heading = array( "product_id", "option_id", "option_value_id", "quantity", "subtract", "price", "price_prefix",'size','color','form', "points", "points_prefix", "weight", "weight_prefix" );
			} else {
				$expected_heading = array( "product_id", "option_id", "option_value", "quantity", "subtract", "price", "price_prefix",'size','color','form', "points", "points_prefix", "weight", "weight_prefix" );
			}
		} else {
			if ($this->config->get('export_import_settings_use_option_value_id')) {
				$expected_heading = array( "product_id", "option", "option_value_id", "quantity", "subtract", "price", "price_prefix",'size','color','form', "points", "points_prefix", "weight", "weight_prefix" );
			} else {
				$expected_heading = array( "product_id", "option", "option_value", "quantity", "subtract", "price", "price_prefix",'size','color','form', "points", "points_prefix", "weight", "weight_prefix" );
			}
		}
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
...
//		}
	...
	protected function getProductOptionValues( $min_id, $max_id ) {
		$language_id = $this->getDefaultLanguageId();
		$sql  = "SELECT ";
		$sql .= "  p.product_id, pov.option_id,pov.size,pov.color,pov.form, pov.option_value_id, pov.quantity, pov.subtract, od.name AS `option`, ovd.name AS option_value, ";
	...
	protected function populateProductOptionValuesWorksheet( &$worksheet, &$price_format, &$box_format, &$weight_format, &$text_format, $min_id=null, $max_id=null ) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('color'),5)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('form'),5)+1);
