<?php
class ControllerExtensionModuleHbSnippets extends Controller {
	
	private $error = array(); 
	
	public function index() {   
		$this->load->language('extension/module/hb_snippets');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('module_hb_snippets', array('module_hb_snippets_status' => 1));	
			$this->session->data['success'] = $this->language->get('text_success');
			$this->response->redirect($this->url->link('extension/module/hb_snippets', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$text_strings = array(
				'heading_title',
				'text_header','text_header_local','text_kg_logo','text_kg_contact','text_kg_social','text_kg_searchbox','text_kg_generate','text_enable','text_disable',
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/hb_snippets', 'user_token=' . $this->session->data['user_token'], true),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('extension/module/hb_snippets', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/hb_snippets', $data));

	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/hb_snippets')) {
...
"priceRange": "'.$price_range.'"
	...
	public function resetoldseopack(){
		$this->db->query("DELETE from `".DB_PREFIX."setting` where `code` = 'hb_snippets_suite' and `key` = 'hb_snippets_local_snippet'");
	}	
